...
function createNode(name: any): any {
...
function connectNodes(
  ...
  const portOut = nodeFrom.addPort(
    new DefaultPortModel(true, `${nodeFrom.name}-out-${count}`, columnName),
  );
  const portTo = nodeTo.addPort(
    new DefaultPortModel(false, `${nodeFrom.name}-to-${count}`),
  );
...
function DemoWidget(props: any) {
  ...
  return <CanvasWidget engine={engine} className="h-screen w-full" />;
}

export default function LineageReactDiagram() {
